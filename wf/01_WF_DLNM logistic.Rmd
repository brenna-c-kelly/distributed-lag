...
# wf-specific pm2.5
cb1.wf_pm25 <- crossbasis(birth_wf_20$max_wf_pm, 
                      lag = 37, 
                      argvar = list(fun = "ns", knots = 4), 
                      arglag = list(fun = "ns", knots = 4))
summary(cb1.wf_pm25) # this describes the crossbasis functions and basis for the predictor and lag
...
# model
model1 <- glm(early_preterm ~ cb1.wf_pm25 + cb1.nonwf_pm25,
                # cb1.pm25 + cb1.wf, # +
                # diabetes_pre + hypertension_pre + momage_c,
...
# model
model1 <- glm(early_preterm ~ cb_days,# +
                # diabetes_pre + hypertension_pre + momage_c,
                #poverty_rate + race_ethn, # adjusting for confounders
                family = "binomial", birth_wf_20)
summary(model1)
...
# prediction
pred1.wf_days <- crosspred(cb_days, model1, at = 0:7,#0:120, 
                           bylag = 1, cumul = TRUE, cen = 0)
